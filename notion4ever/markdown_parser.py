...
def column_list(information: list) -> str:
    """
    input: item:dict = {"checked":bool, "test":str}
    """
    print("hey")
    ...
    if (
        ...
        if block_type in ["child_page", "child_database", "db_entry"]:
            ...
            if structured_notion["pages"][block["id"]]["emoji"]:
                ...
                outcome_block = f"""[<span class="miniicon"> <img src="{icon}"></span> <span class="page-title-link">{outcome_block}</span>"""
            else:
                outcome_block = f"[{outcome_block}"
            if block_type == "child_database":
                # Actually render out the database:
                if "md_content" in structured_notion["pages"][block["id"]]:
                    page_inline = generate_page_inline(block["id"], structured_notion, {
                        "templates_dir": "./_templates",
                        "output_dir": "./_site",
                        "build_locally": False,
                        "site_url": os.environ.get("SITE_URL"),
                    })
                    outcome_block = page_inline
                ...
                outcome_block = (
                    block_type_map[block_type](
                        information_collector(
                            block[block_type], structured_notion, page_id
                        )
                    )
                    + "\n\n"
                )
                ...
                if block_type == "table":
                    ...
                    for block in child_blocks:
                        ...
                        if block["type"] == "heading_1":
                            print(f"DEPTH {depth}")
                            depth = 0
                        block_md = block_convertor(
                            block, depth, structured_notion, page_id
                        )
...
def parse_markdown(raw_notion: dict, structured_notion: dict):
    for page_id, page in raw_notion.items():
        if not "md_content" in structured_notion["pages"][page_id]:
            structured_notion["pages"][page_id]["md_content"] = ""
        page_md = blocks_convertor(
            raw_notion[page_id]["blocks"], structured_notion, page_id
        )
